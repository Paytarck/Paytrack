...
if (submitFeedbackBtn) {
    submitFeedbackBtn.onclick = async () => {
        ...
        if (!text && selectedStars === 0) {
            showNotification("Please add a rating or a message.", "error");
            return;
        }

        // UI Loading
        submitFeedbackBtn.disabled = true;
        document.getElementById('feedbackSpinner').classList.remove('hidden');
        document.getElementById('feedbackBtnText').textContent = "Submitting...";
        ...
        try {
            ...
            renderFeedbackPreviews();
            // Reset star UI
            starContainer.querySelectorAll('.star-btn').forEach(s => s.className = 'far fa-star star-btn cursor-pointer text-gray-300');
            
        } catch (e) {
            showNotification("Failed to send feedback.", "error");
        } finally {
            submitFeedbackBtn.disabled = false;
            document.getElementById('feedbackSpinner').classList.add('hidden');
