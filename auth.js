    ...
    try {
        ...
        if (!userSnap.exists()) {
            ...
            await setDoc(userRef, {
                ...
                pin: defaultHashedPin, authProvider: 'google', createdAt: new Date().toISOString(),
        ...
        // CRITICAL FIX: Wait for download to finish COMPLETELY before doing anything else
        await downloadUserData(username);
        
        // Only sync AFTER we are sure we have the latest from cloud
...
export async function loginUser(username, pin) {
    ...
    // CRITICAL FIX: Wait for download
    await downloadUserData(username);
    await syncLocalCardsToCloud();
    return true;
}
