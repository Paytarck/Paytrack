...
export function subscribeToProjectCard(cardNumber, onUpdateCallback) {
    ...
    return onSnapshot(cardRef, { includeMetadataChanges: true }, (docSnap) => {
        // If the data is currently being sent from THIS device to the cloud, 
        // ignore it. Do not let it overwrite the local UI.
...
export async function updateGlobalCard(project, projectData) {
    ...
    // Use updateDoc instead of setDoc to prevent wiping out 
    // parts of the project data that weren't sent in this specific click.
    const payload = {
    ...
    try {
        // We use setDoc with { merge: true } to ensure we don't delete 
        // the card if it exists, but only update the fields provided.
