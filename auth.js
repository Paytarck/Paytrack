 ...
 */
// --- Improved subscribeToUserData in auth.js ---
export function subscribeToUserData(username, onUpdateCallback) {
    ...
    return onSnapshot(userRef, { includeMetadataChanges: true }, (docSnap) => {
        // Skip updates we just sent to avoid "jumping" UI
        ...
        if (docSnap.exists()) {
            ...
            // Security: If cloud has projects, update local storage
            if (appData.projects && appData.projects.length > 0) {
                localStorage.setItem('allTrackerProjects', JSON.stringify(appData.projects));
            }
            
            // Sync all specific project detail keys
...
export async function downloadUserData(username) {
    ...
    if (userSnap.exists()) {
        ...
        // Instead, we specifically update the keys we need.
        if (appData.projects) {
            localStorage.setItem('allTrackerProjects', JSON.stringify(appData.projects));
        }
        if (appData.globalSettings) {
            localStorage.setItem('dashboardGlobalSettings', JSON.stringify(appData.globalSettings));
        }
...
export function subscribeToUserData(username, onUpdateCallback) {
    ...
    return onSnapshot(userRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            // If the phone has data but the cloud is empty, STOP. 
            // This prevents the "reset to 0" bug.
            const localProjects = JSON.parse(localStorage.getItem('allTrackerProjects')) || [];
            if (localProjects.length > 0 && (!appData.projects || appData.projects.length === 0)) {
                console.warn("Sync blocked: Cloud is empty but Phone has data. Protecting local data.");
                return; 
            }
