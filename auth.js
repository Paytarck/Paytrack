...
export async function syncDataToCloud() {
    ...
    // GUARD: Load projects from local storage
    const projects = JSON.parse(localStorage.getItem('allTrackerProjects')) || [];
    
    // NEW PROTECTION: If local projects are empty, check if we have ever synced before.
    // This prevents a new login on a laptop from wiping the cloud data.
    if (projects.length === 0) {
        const userRef = doc(db, "users", username);
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
            const cloudData = userSnap.data().data || {};
            // If the cloud has projects but our local doesn't, STOP the sync to prevent wiping.
            if (cloudData.projects && cloudData.projects.length > 0) {
                console.warn("Sync blocked: Local storage is empty but cloud has data. Avoiding wipeout.");
                return; 
            }
        }
    }
