...
export async function syncDataToCloud() {
    ...
    // PROTECTION: If local is empty, we must check the cloud before uploading
    // This stops a fresh laptop login from wiping out your mobile data
    if (localProjects.length === 0) {
        const userSnap = await getDoc(userRef);
        if (userSnap.exists()) {
            const cloudData = userSnap.data().data || {};
            if (cloudData.projects && cloudData.projects.length > 0) {
                console.warn("Blocking sync: Local storage is empty but cloud has projects. Download required first.");
                return; // STOP the sync to prevent wiping the cloud
            }
        }
    }
