...
if (username) {
    subscribeToUserData(username, (newData) => {
        // This runs when the cloud changes
        loadInitialData(); // Reloads appState and settings from localStorage
        updateSummary();   // Updates the top cards ($ totals)
        if (currentPage === 'history') renderHistoryTable(); // Updates the list
    });

    // Also listen to the specific project card for sharing
    const allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
    ...
    if (projectInfo && projectInfo.cardNumber) {
        subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
            // Force update local storage with the shared data
            localStorage.setItem(INSTALLMENT_STORAGE_KEY, JSON.stringify(remoteData.installment));
            localStorage.setItem(EXPENSE_STORAGE_KEY, JSON.stringify(remoteData.expense));
            
            loadInitialData(); // Refresh everything on screen
            updateSummary();
