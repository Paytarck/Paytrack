...
async function triggerCloudSync() {
    ...
    // REMOVE the "if (appState.payments.length === 0)" line.
    // We want to sync metadata and settings even if there are no payments yet.

    try {
        const auth = await import('./auth.js');
        await auth.syncDataToCloud();
        await autoUpdateGlobalCard(); // Ensure shared cards are also updated
        console.log("PayTrack: Cloud Sync Triggered");
    } catch (e) {
        console.log("Sync skipped/failed");
    }
}
...
if (username) {
    subscribeToUserData(username, (newData) => {
        ...
        // Find the current project info to get the card number
   const allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
const projectInfo = allProjects.find(p => p.id == currentProjectId);

// Check if this project has a linked card number
if (projectInfo && projectInfo.cardNumber) {
    console.log("Listening for shared updates on card:", projectInfo.cardNumber);
    
    subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
        // Prevent overwriting if local storage is already ahead (handled by hasPendingWrites in auth.js)
        if (!remoteData) return;

        console.log("Remote update received from shared card!");
        ...
        // Refresh UI to show the other person's changes
        loadInitialData();
    });
}
...
async function saveData() {
    ...
    // UPDATE: Sync the project name back to the main list
    let allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
    let pIdx = allProjects.findIndex(p => p.id == currentProjectId);
    if (pIdx !== -1) {
        allProjects[pIdx].name = appState.projectName;
        localStorage.setItem(PROJECTS_KEY, JSON.stringify(allProjects));
    }

    localStorage.setItem(SETTINGS_KEY, JSON.stringify(currentSettings));
    
    // Now trigger the syncs
