...
async function saveData() {
    try {
        ...
        // This ensures that when we read the data in the next step, it is FRESH.
        ...
        // 2. Prepare the data package for the Cloud
        // We read from LocalStorage to ensure we have the most stable version of all modes
        const instData = JSON.parse(localStorage.getItem(INSTALLMENT_STORAGE_KEY));
        const expData = JSON.parse(localStorage.getItem(EXPENSE_STORAGE_KEY));
        ...
        if (currentProjectObj) {
            ...
            // 3. PUSH TO FIREBASE (Using the top-level imports directly)
            // No more 'await import' here!
            await updateGlobalCard(currentProjectObj, fullData);
            await syncDataToCloud(); 
            
            console.log("✅ Sync: Data pushed to Firebase with timestamp:", now);
        }
    } catch (error) {
