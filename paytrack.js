...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("📡 PayTrack: Listening for cloud updates on card:", projectInfo.cardNumber);
    
    subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
        if (!remoteData) return;

        console.log("☁️ Real-time project update received!");
        ...
        // 2. Identify which mode the remote data is in
        const isExpenseMode = remoteData.settings?.expenseMode || false;
        const incomingData = isExpenseMode ? remoteData.expense : remoteData.installment;

        if (incomingData) {
            // 3. Update LIVE memory variables directly from the cloud data
            appState = { ...appState, ...incomingData };
            currentSettings = { ...currentSettings, ...remoteData.settings };
            
            // 4. CRITICAL: Recalculate totals (math) based on new payments
            recalculateTotals(); 

            // 5. Force UI to refresh Summary Cards and Progress Bar
            updateUIMode(); 
            updateSummary();
            
            // 6. Refresh other pages if the user is currently looking at them
...
async function saveData() {
    ...
    if (currentProjectObj) {
        ...
        // Push to individual card (Real-time sync)
        await updateGlobalCard(currentProjectObj, fullData);
        
        // Push to User profile (Dashboard sync)
