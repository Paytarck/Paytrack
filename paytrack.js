...
async function saveData() {
    try {
        // 1. Generate a timestamp for this specific action
        const now = Date.now();
        appState.lastLocalUpdate = now; 

        // 2. Save everything to LocalStorage immediately so the UI stays updated
        const dataKey = currentSettings.expenseMode ? EXPENSE_STORAGE_KEY : INSTALLMENT_STORAGE_KEY;
        localStorage.setItem(dataKey, JSON.stringify(appState));
        localStorage.setItem(SETTINGS_KEY, JSON.stringify(currentSettings));

        // 3. Prepare the full package for Firebase
        const allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
        const currentProjectObj = allProjects.find(p => p.id == currentProjectId);
        
        if (currentProjectObj) {
            // We gather ALL data (Installment, Expense, and Settings) 
            // to ensure we don't accidentally wipe one mode while saving the other.
            const fullData = {
                ...
                lastUpdated: now // CRITICAL: This is our shield
            };
            ...
            console.log("✅ Payment synced to Cloud with timestamp:", now);
        }
    } catch (error) {
        console.error("❌ Sync failed during payment:", error);
    }
}
