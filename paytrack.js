...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Remove the strict TS check for a moment to ensure updates flow
    // and use metadata to prevent the app from overwriting its own changes
    console.log("📥 Real-time update received from Cloud...");

    // 1. Update ALL Local Storage keys immediately
    ...
    // 2. Update memory state for the active mode
    currentSettings = remoteData.settings || currentSettings;
    appState = currentSettings.expenseMode ? remoteData.expense : remoteData.installment;

    // 3. FORCE UI REFRESH - This is the "Magic" for Android
    // Update Text/Headers
    if (elements.projectNameHeader) elements.projectNameHeader.textContent = `Project: ${appState.projectName}`;
    if (elements.projectName) elements.projectName.value = appState.projectName;
    ...
    // Recalculate totals (Paid, Pending, Progress Bar)
    recalculateTotals();
    updateSummary();
    updateUIMode();

    // IF the user is currently looking at the History page, redraw the table!
    if (currentPage === 'history') {
        renderHistoryTable();
    }
    
    // IF the user is looking at the Details page, refresh it!
    if (currentPage === 'detail' && currentDetailId) {
        showTransactionDetail(currentDetailId);
    }

    // Refresh translations/language
    if (typeof translatePage === 'function') translatePage();

    // --- PASTE THE STATUS INDICATOR CODE HERE ---
    const status = document.getElementById('cloudStatus');
    if(status) {
        status.classList.remove('opacity-0'); // Make it visible
        setTimeout(() => {
            status.classList.add('opacity-0'); // Hide it again after 2 seconds
        }, 2000);
    }
    
    console.log("Real-time UI Update Triggered");
    
    console.log("✅ UI Redraw Complete (No refresh needed)");
});
    }
    };
...
async function loadInitialData() {
    // REFRESH the global settings from the dashboard key
    const globalData = JSON.parse(localStorage.getItem('dashboardGlobalSettings')) || {};
    
    // Update the local globalSettings variable used for formatting
...
async function saveData() {
    try {
        ...
        // Update timestamp in state before saving
        ...
        if (currentProjectObj && currentProjectObj.cardNumber) {
            const fullData = {
                ...
                lastUpdated: now // Global package timestamp
            };
...
if (elements.receiptModal) {
    ...
    elements.btnSelectManual.onclick = () => {
        // 1. Hide selection modal
        elements.inputTypeSelectionModal.classList.add('hidden');
        
        // 2. Open the form (pendingTransactionType was set when clicking Add Income/Expense)
        console.log("Opening manual form for:", pendingTransactionType);
...
async function compressImage(base64Str) {
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            const MAX_WIDTH = 800; // Resize to 800 pixels wide
            let width = img.width;
            ...
            // Save as a medium-quality JPEG to save space
