...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (projectInfo && projectInfo.cardNumber) {
        subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
            ...
            // --- THE SHIELD: Timestamp Check ---
            // If the data coming from the cloud is OLDER than our last local save, 
            // IGNORE it. This stops the "disappearing data" bug.
            const remoteTS = remoteData.lastUpdated || 0;
            const localTS = appState.lastLocalUpdate || 0;
            
            if (remoteTS <= localTS) {
                console.log("🛡️ Ignored older data from cloud to prevent overwriting new changes.");
                return; 
            }

            console.log("☁️ Fresh data received from another device. Updating...");
            ...
            if (incomingData) {
                ...
                appState.lastLocalUpdate = remoteTS; // Sync the timestamp
...
async function saveData() {
    try {
        ...
        // Mark exactly when this change happened
        ...
        if (currentProjectObj && currentProjectObj.cardNumber) {
            // Include the timestamp in the cloud payload
            const fullData = {
                ...
                lastUpdated: appState.lastLocalUpdate 
            };
