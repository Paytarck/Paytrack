...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const startSync = (projectList) => {
        ...
        if (projectInfo && projectInfo.cardNumber) {
            console.log("📡 Sync Started for Card:", projectInfo.cardNumber);
    ...
    // 3. Logic to wait for the Card Number if it's not locally available yet
    ...
    if (foundLocal && foundLocal.cardNumber) {
        ...
        // If not found locally, wait for the User Data subscription to provide it
        const username = localStorage.getItem('paytrackUsername');
        subscribeToUserData(username, (newData) => {
            if (newData.projects) {
                startSync(newData.projects);
            }
        });
    }
});
...
async function saveData() {
    ...
    // 2. Prepare the full data for the Cloud Card
    const allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
    const currentProjectObj = allProjects.find(p => p.id == currentProjectId);
    
    if (currentProjectObj && currentProjectObj.cardNumber) {
        // We gather everything for THIS specific project
        const fullData = {
        ...
        // 3. Update the Global Card (This is what Device B listens to)
        await updateGlobalCard(currentProjectObj, fullData);
        
        // 4. Update User Profile (This only updates the project list/names)
