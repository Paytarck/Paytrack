...
const username = localStorage.getItem('paytrackUsername');
// paytrack.js inside DOMContentLoaded
if (username) {
    // 1. Listen for general profile changes (Project List)
    subscribeToUserData(username, (newData) => {
        loadInitialData(); // Reload appState from local storage
        updateSummary();   // Update the $ totals on screen
    });

    // 2. Listen for specific project updates (Shared Card)
    const allProjects = JSON.parse(localStorage.getItem('allTrackerProjects')) || [];
    ...
    if (projectInfo && projectInfo.cardNumber) {
        subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
            ...
            console.log("Shared sync received from other device.");

            // CRITICAL FIX: Save the shared data to the CORRECT project keys
...
async function saveData() {
    ...
    // 2. Sync Project Name to the main dashboard list
    let allProjects = JSON.parse(localStorage.getItem('allTrackerProjects')) || [];
    let pIdx = allProjects.findIndex(p => p.id == currentProjectId);
    if (pIdx !== -1) {
        allProjects[pIdx].name = appState.projectName;
        localStorage.setItem('allTrackerProjects', JSON.stringify(allProjects));
    }

    // 3. Trigger Cloud Sync
    // We pass the data directly to ensure the cloud gets the LATEST values
    const fullData = {
        installment: !currentSettings.expenseMode ? appState : JSON.parse(localStorage.getItem(INSTALLMENT_STORAGE_KEY)),
        expense: currentSettings.expenseMode ? appState : JSON.parse(localStorage.getItem(EXPENSE_STORAGE_KEY)),
    ...
    if (username) {
        const auth = await import('./auth.js');
        // Update the owner's backup
        await auth.syncDataToCloud(); 
        // Update the shared card for Device 2
        const currentProjectObj = allProjects.find(p => p.id == currentProjectId);
        if (currentProjectObj) {
            await auth.updateGlobalCard(currentProjectObj, fullData);
        }
