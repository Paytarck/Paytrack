...
function loadInitialData() {
    ...
    const savedSettings = JSON.parse(localStorage.getItem(SETTINGS_KEY)) || { theme: 'blue', expenseMode: currentProject.type === 'finance' };
...
function openProjectDetails() {
    ...
    if (data.receipt) {
        elements.viewDetailReceiptImg.src = data.receipt;
        elements.viewDetailReceiptContainer.classList.remove('hidden');
        elements.viewDetailReceiptImg.onclick = () => {
            elements.receiptModalImage.src = data.receipt;
...
function setupEventListeners() {
    ...
    if (elements.initReceipt) {
        elements.initReceipt.addEventListener('change', () => {
            const file = elements.initReceipt.files[0];
            if (file) {
                const reader = new FileReader();
                reader.onload = (e) => {
                    elements.initReceiptPreview.src = e.target.result;
    ...
    if (elements.removeInitReceiptBtn) {
     ...
     // Navigation Buttons
        ...
        // Fix for Project Agreement Modal Close Button
    if (elements.closeProjectDetailsBtn) {
        elements.closeProjectDetailsBtn.addEventListener('click', () => {
    ...
    //add payment for installment tracker
    // KEEP ONLY THIS ONE for the main payment form
elements.paymentForm.addEventListener('submit', (e) => { 
    e.preventDefault(); 
    e.stopPropagation(); // Prevents the event from triggering other listeners
...
elements.btnSaveTotalAmount.addEventListener('click', (e) => {
    ...
    if (closeBtn) {
        closeBtn.addEventListener('click', () => {
            ...
            // Re-enable body scroll if you disabled it
            document.body.style.overflow = 'auto'; 
        });
    }

    // Close on clicking outside the card
    const modalBg = document.getElementById('projectDetailsModal');
    modalBg.addEventListener('click', (e) => {
        if (e.target === modalBg || e.target.classList.contains('min-h-screen')) {
...
async function handleProjectSetup() {
    ...
    const receiptFile = elements.initReceipt.files[0];
    ...
    // 2. SAVE AS METADATA (Not as a transaction in payments list)
    appState.projectMetaData = {
        name: appState.projectName,
        totalCost: total,
        agreementDate: date,
