let isInitialDownloading = false;
...
async function triggerCloudSync() {
    if (isInitialDownloading) return; // Prevent overwriting cloud with empty local data
...
const username = localStorage.getItem('paytrackUsername');
if (username) {
    // Listener 1: General User Profile (Settings, Project List)
    subscribeToUserData(username, (newData) => {
        console.log("Profile updated from cloud");
        loadInitialData(); // Refresh labels/settings
    });

    // Listener 2: Specific Project Data (Payments/History)
    const allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
    ...
    if (projectInfo && projectInfo.cardNumber) {
        subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
            ...
            console.log("Payments updated from shared card");
