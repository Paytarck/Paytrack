...
function loadInitialData() {
    // 1. Load project-specific settings (Theme, Mode, etc.)
    ...
    // 2. Load global settings (Currency, Language)
    globalSettings = JSON.parse(localStorage.getItem(GLOBAL_SETTINGS_KEY)) || {};
    ...
    // 3. Determine if we are in Expense or Installment mode
    ...
    // 4. Verify project exists in the dashboard list
    ...
    // 5. Load the actual payment/transaction data
    if (savedData) {
    ...
    // 6. Apply visuals and custom data
    applyTheme(currentSettings.theme);
    loadCustomBanks();

    // --- FIX: RESTORE VISIBILITY STATE FROM STORAGE ---
    // This prevents the screen from blurring again and again during sync
    ...
    // Apply the blurred or visible state to the cards immediately
    updateVisibilityUI(); 
}
...
function showInputSelectionModal(type) {
    ...
    const modal = elements.inputTypeSelectionModal;
...
function showPage(page) {
    ...
    const pageMap = { main: elements.mainPage, history: elements.historyPage, settings: elements.settingsPage, detail: elements.detailPage, yearlyChart: elements.yearlyChartPage, monthlyChart: elements.monthlyChartPage, card: elements.cardPage, shortcuts: elements.shortcutTransactionsPage, autoTransactionsList: elements.autoTransactionsListPage };
    if (pageMap[page]) {
        ...
        if (page === 'autoTransactionsList') renderAutoTransactionsPage();
    }
}
...
function updateUIMode() {
    ...
    if (mainHeader) {
        const logoHTML = `<img id="mainPageDesktopLogo" src="Paytrack-icon.png" alt="Tracker icon" class="mr-3 header-icon desktop-logo" height="60" width="60"/>`;
