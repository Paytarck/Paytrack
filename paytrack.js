...
async function triggerCloudSync() {
    if (localStorage.getItem('paytrackUsername')) {
        try {
            // Dynamic import to allow this script to run without type="module" in HTML if needed
            const auth = await import('./auth.js');
            await auth.syncDataToCloud();
            console.log("PayTrack: User Profile Auto-sync triggered");
        } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (username) {
        subscribeToUserData(username, (newData) => {
            // Only refresh if the user isn't currently typing in a modal or editing
            const isModalOpen = !elements.transactionModal.classList.contains('hidden');
            const isEditing = !!elements.editPaymentId.value;

            if (!isModalOpen && !isEditing) {
                console.log("Tracker page synced from cloud!");
                
                // 1. Refresh the app state from the newly updated LocalStorage
                loadInitialData(); 
                
                // 2. Re-render whichever view the user is looking at
                if (currentPage === 'history') renderHistoryPage();
                if (currentPage === 'yearlyChart') renderYearlyCharts();
                if (currentPage === 'card') renderCardPage();
                if (currentPage === 'shortcuts') renderShortcutsPage();
                
                // 3. Update the visible amounts (Show/Hide)
                updateVisibilityUI();
            }
        });
    }
