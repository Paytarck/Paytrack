...
function loadInitialData() {
    // 1. Get the main project list to find the type created in Dashboard
    ...
    if (!savedSettings) {
        // FIRST TIME OPENING: Set mode based on what user selected in Dashboard
        currentSettings = {
            expenseMode: (currentProject.type === 'finance'), // Sets true if 'expense'
            theme: 'blue',
        ...
        // Not the first time: Load existing settings
        currentSettings = savedSettings;
        
        // PROTECTION: If user somehow has mismatched data, sync it once
        // (Optional: ensures a 'finance' project actually opens in finance mode)
        if (currentProject.type === 'expense' && currentSettings.expenseMode === undefined) {
    ...
    updateUIMode(); // This will now correctly show the Finance or Installment UI
    updateSummary();
    updateSettingsUI();
}
...
function updateUIMode() {
    ...
    const logoHTML = `<img src="${logoSrc}" class="header-icon mr-3" height="50" width="50" style="display:inline-block; vertical-align:middle; border-radius:50%; background:white; padding:4px;"/>`;
