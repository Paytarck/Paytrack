...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (remoteTS > localTS) {
        console.log("📥 Receiving deep sync (Receipts & Metadata)...");
        ...
        // This line is crucial: it captures the 'projectMetaData' including synced receipts
        const newState = currentSettings.expenseMode ? remoteData.expense : remoteData.installment;
        appState = newState;

        // 3. Update UI
        if (elements.projectName) elements.projectName.value = appState.projectName;
        if (elements.totalCost) elements.totalCost.value = appState.totalAmount;
        
        recalculateTotals();
        updateSummary();

        // 4. FIX: If the Agreement Modal is open, close it or alert the user
        // so they see the new images when they reopen it.
        if (!elements.projectDetailsModal.classList.contains('hidden')) {
            showNotification("Project details updated from cloud.", "success");
            // Optional: auto-refresh the modal if it's open
            openProjectDetails(); 
        }
    }
});
