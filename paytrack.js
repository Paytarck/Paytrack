...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (projectInfo && projectInfo.cardNumber && !window.hasStartedSync) {
        ...
        subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
            ...
            const localTS = appState.lastLocalUpdate || 0;
            
            // If the cloud data is older or equal to what we just saved, IGNORE IT.
            // This stops the "Add Payment" from being overwritten by the old server state.
            if (remoteTS <= localTS) {
                return; 
            }

            console.log("☁️ New data from another device detected. Updating local state...");
            ...
            if (remoteData.settings) {
                localStorage.setItem(SETTINGS_KEY, JSON.stringify(remoteData.settings));
            ...
            if (incomingData) {
                const preservedName = appState.projectName;
                appState = { ...appState, ...incomingData };
                appState.projectName = preservedName;
                appState.lastLocalUpdate = remoteTS;
...
async function loadInitialData() {
    ...
    // 2. CLOUD FETCH FALLBACK: If local is empty, pull from Cloud
    if (!savedData && currentProject.cardNumber) {
        console.log("Empty local data. Fetching project history from cloud...");
        const { downloadProjectData } = await import('./auth.js');
        const cloudData = await downloadProjectData(currentProject.cardNumber);
        
        if (cloudData) {
            // Save what we found in cloud to local storage
            if (cloudData.installment) localStorage.setItem(INSTALLMENT_STORAGE_KEY, JSON.stringify(cloudData.installment));
            if (cloudData.expense) localStorage.setItem(EXPENSE_STORAGE_KEY, JSON.stringify(cloudData.expense));
            if (cloudData.settings) localStorage.setItem(SETTINGS_KEY, JSON.stringify(cloudData.settings));
            
            // Reload the local variable
            savedData = currentSettings.expenseMode ? cloudData.expense : cloudData.installment;
        }
    }
...
async function saveData() {
    try {
        const now = Date.now();
        appState.lastLocalUpdate = now; 

        // 1. Save to LocalStorage for immediate UI persistence
        ...
        // 2. Identify the project for Cloud Sync
        const allProjects = JSON.parse(localStorage.getItem(PROJECTS_KEY)) || [];
        // We use == to handle potential string/number ID mismatch
        const currentProjectObj = allProjects.find(p => p.id == currentProjectId);
        ...
        if (currentProjectObj && currentProjectObj.cardNumber) {
            ...
            const expData = JSON.parse(localStorage.getItem(EXPENSE_STORAGE_KEY));
            const sets = JSON.parse(localStorage.getItem(SETTINGS_KEY));
            const shortcutData = JSON.parse(localStorage.getItem(SHORTCUTS_STORAGE_KEY));
            ...
            // 3. PUSH TO FIREBASE
            // This updates the global card (transactions)
            await updateGlobalCard(currentProjectObj, fullData);
            
            // This updates the user profile (project list)
            await syncDataToCloud(); 
            
            console.log("✅ Cloud Sync Success:", now);
        } else {
            console.warn("⚠️ Sync skipped: Project object or CardNumber missing.");
        }
    } catch (error) {
...
async function finishTransaction(message, isShortcut = false) {
    ...
    // Await the sync so we don't reload or change state before it finishes
    await saveData(); 
