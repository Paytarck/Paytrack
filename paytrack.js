...
async function loadInitialData() {
    ...
    // FIX: Ensure all required setting arrays are initialized immediately
    ...
    // CLOUD RECOVERY: If local is empty, pull from Cloud
    if (!savedData && currentProject.cardNumber) {
        try {
            ...
            console.error("Cloud fetch failed, using defaults");
        }
    }
    ...
    // UI REFRESH: This unhides the buttons and fixes the colors
...
async function saveData() {
    try {
        // 1. THE GUARD: If the app hasn't loaded any data yet, STOP.
        // This prevents Device B from wiping the cloud on refresh.
        if (appState.totalAmount === 0 && appState.payments.length === 0) {
            console.warn("⚠️ Save blocked: Local data is empty. Waiting for cloud sync...");
            return; 
        }
