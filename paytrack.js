import { subscribeToUserData, subscribeToProjectCard, updateGlobalCard, syncDataToCloud } from './auth.js';
// --- CLOUD SYNC HELPERS ---
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (projectInfo && projectInfo.cardNumber) {
        console.log("📡 Transaction Sync Active:", projectInfo.cardNumber);
        
        subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
            if (!remoteData) return;
            
            console.log("☁️ Remote Project Update Received");

            // 1. Sync LocalStorage (for persistence)
            ...
            // 2. CRITICAL: Update the 'currentSettings' variable in memory
            if (remoteData.settings) {
                localStorage.setItem(SETTINGS_KEY, JSON.stringify(remoteData.settings));
                currentSettings = remoteData.settings;
            }

            // 3. Update the live appState based on the updated mode
            const isExpenseMode = currentSettings.expenseMode;
            const incomingData = isExpenseMode ? remoteData.expense : remoteData.installment;

            if (incomingData) {
                // Preserve the Project Name from the dashboard list
                const preservedName = appState.projectName;
                
                // Deep merge incoming data into the active state
                appState = { ...appState, ...incomingData };
                appState.projectName = preservedName;
                
                // 4. FORCE UI REFRESH
                updateUIMode();     // Re-evaluates Finance vs Installment layout
                recalculateTotals(); // Re-runs the math on new transactions
                updateSummary();    // Updates the big number cards
                
                // 5. Refresh specific pages if the user is looking at them
                ...
                console.log("✅ Project UI Synced Successfully");
            }
        });
    }
    };
...
async function saveData() {
    try {
        ...
        if (currentProjectObj && currentProjectObj.cardNumber) {
            ...
            // 3. Push to Real-time Card
            await updateGlobalCard(currentProjectObj, fullData);
            
            // 4. Push to User Profile
            await syncDataToCloud(); 
        }
    } catch (error) {
        console.error("Critical Sync Error:", error);
        // We don't alert the user here so they can keep working offline
    }
}
