...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Identify which mode the remote data is in
    ...
    if (incomingData) {
        // 2. Sync LocalStorage for persistence
...
async function saveData() {
    ...
    if (currentProjectObj) {
        ...
        // 2. Update the Real-time Card (for the PayTrack page)
        await auth.updateGlobalCard(currentProjectObj, fullData);
        
        // 3. Update the User Profile (for the Dashboard/Sync)
        await auth.syncDataToCloud(); 
    }
}
