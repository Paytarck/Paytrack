import { subscribeToUserData } from './auth.js';
// --- CLOUD SYNC HELPERS ---
...
async function triggerCloudSync() {
    ...
    // DATA PROTECTION: If the payments list is empty but we just loaded the page,
    // wait before syncing so we don't accidentally wipe the cloud with "nothing".
    if (appState.payments.length === 0) {
        console.log("Sync delayed: Waiting for data to load...");
        return;
    }
    ...
    try {
        ...
        // 2. Sync the actual data INSIDE this specific project (The 7 rs you added)
        await auth.syncLocalCardsToCloud(); 
        
        console.log("PayTrack: Full Cloud Sync Complete");
    } catch (e) {
...
if (username) {
    subscribeToUserData(username, (newData) => {
        ...
        // 1. Re-load the data from LocalStorage (which auth.js just updated)
        loadInitialData(); 
        
        // 2. If the user is looking at history or charts, refresh those too
        ...
        // 3. Make sure the blur/visible state is correct
        updateVisibilityUI();
    });
}
});
...
function loadInitialData() {
    // 1. Get the main project list to find the project created in Dashboard
    ...
    // --- FEATURE: FIXED & AUTO-SYNCED PROJECT NAME ---
    // Every time the page loads, we grab the LATEST name from the Dashboard list
    appState.projectName = currentProject.name;
    elements.projectName.value = currentProject.name;
    
    // Lock the field so the user cannot change it manually here
    elements.projectName.readOnly = true;
    
    // Apply visual styles to show it is a fixed/locked field
    const isDark = document.body.classList.contains('theme-dark');
    elements.projectName.style.backgroundColor = isDark ? '#1f2937' : '#f3f4f6';
    elements.projectName.style.cursor = 'not-allowed';
    elements.projectName.style.opacity = '0.8';

    // 2. Load settings, OR create them if this is a new project
    const savedSettings = JSON.parse(localStorage.getItem(SETTINGS_KEY));
    
    if (!savedSettings) {
        // FIRST TIME OPENING: Set mode based on Dashboard selection
        currentSettings = {
            expenseMode: (currentProject.type === 'finance' || currentProject.type === 'finance'),
            theme: 'blue',
        ...
        // Robust check: Ensure mode matches the Project Type if dashboard was changed
        if (currentProject.type === 'finance') {
            currentSettings.expenseMode = true;
        }
    }

    // 3. Initialize the application state based on the mode
    const isExpenseMode = currentSettings.expenseMode;
    appState = getNewState(isExpenseMode);
    
    // Ensure state always uses the latest Dashboard name
    appState.projectName = currentProject.name;
    ...
    if (savedData) {
        // Merge saved payments/totals with current state
        appState = { ...appState, ...savedData };
        // Overwrite the saved name with the Dashboard name in case it was edited
        appState.projectName = currentProject.name;
    }
    ...
    // --- FEATURE: AUTOMATIC TODAY DATE ---
    // Sets the date picker to the current user's date by default
    setTodayDate();
}
...
function saveData() {
    ...
    // 1. Sync User Profile (List of projects etc)
    triggerCloudSync();

    // 2. Update the Global Card for this project (Cloud Backup)
    autoUpdateGlobalCard();
}
...
elements.btnSaveTotalAmount.addEventListener('click', () => {
...
function updateTotalCostFieldBehavior() {
    ...
    // PROJECT NAME IS ALWAYS READONLY NOW
    elements.projectName.readOnly = true; 
    ...
    const readOnlyStyle = (element, isReadOnly) => {
        const bgColor = isReadOnly ? (document.body.classList.contains('theme-dark') ? '#1f2937' : '#f3f4f6') : '';
        const cursor = isReadOnly ? 'not-allowed' : '';
        element.style.backgroundColor = bgColor;
        element.style.cursor = cursor;
    };

    readOnlyStyle(elements.projectName, true); // Always applied
