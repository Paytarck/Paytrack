    ...
    const startSync = (projectList) => {
        ...
        if (projectInfo && projectInfo.cardNumber) {
            ...
            subscribeToProjectCard(projectInfo.cardNumber, (remoteData) => {
                ...
                // 1. Sync LocalStorage keys for both modes to keep them ready
                ...
                // 2. Update the live appState based on the CURRENT active mode
                ...
                if (incomingData) {
                    // Merge data but preserve the Project Name from the dashboard list
                    const preservedName = appState.projectName;
                    appState = { ...appState, ...incomingData };
                    appState.projectName = preservedName;
                    
                    // 3. Re-calculate math and refresh UI
                    recalculateTotals(); 
                    updateSummary();
                    
                    // 4. Refresh specific pages if the user is looking at them
...
function updateUIMode() {
    ...
    // 2. Installment Setup vs Standard Form Logic
    if (!isExpenseMode) {
    ...
    // 3. Update Header Text safely (Keep the logos intact)
