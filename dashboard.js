...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    // --- AGGRESSIVE SYNC FOR LAPTOPS ---
    if (username) {
        // 1. Immediately try to download data first
        const authModule = await import('./auth.js');
        await authModule.downloadUserData(username);
        renderProjects(); // Show projects immediately after download

        // 2. Then start the live listener
