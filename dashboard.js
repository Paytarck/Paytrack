...
nSearch?.addEventListener('click', () => {
    ...
    if (isHidden) {
        ...
        // Scroll below the sticky header, not behind it
        requestAnimationFrame(() => {
            const headerEl = document.querySelector('.header-glass');
            const offset = headerEl ? headerEl.offsetHeight + 12 : 90;
            const top = findPanel.getBoundingClientRect().top + window.scrollY - offset;
...
nAdd?.addEventListener('click', () => {
    ...
    if (isHidden) {
        ...
        requestAnimationFrame(() => {
            const headerEl = document.querySelector('.header-glass');
            const offset = headerEl ? headerEl.offsetHeight + 12 : 90;
            const top = addPanel.getBoundingClientRect().top + window.scrollY - offset;
            window.scrollTo({ top, behavior: 'smooth' });
...
importCardForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // cloudCard IS the raw Firestore document returned by fetchProjectByCard.
        // ownerUsername and projectType are now stored at the top level of the card
        // document by updateGlobalCard in auth.js.
        //
        // Bug fix 1 — type: use projectType field stored on card; fall back to
        //   expenseMode flag; never guess wrong.
        // Bug fix 2 — importedFrom: use the card's ownerUsername (the person who
        //   CREATED the card), NOT the current user who is importing it.
        const resolvedType = cloudCard.projectType
            || (cloudCard.fullData?.settings?.expenseMode ? 'finance' : 'installment');

        const resolvedOwner = cloudCard.ownerUsername || null;
        ...
        const newProjectEntry = {
            ...
            type: resolvedType,
            cardNumber: cardNum,
            importedFrom: resolvedOwner   // null if owner never synced — badge simply won't show
        };
