...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (username) {
        import('./auth.js').then(auth => {
            // First: Start the real-time listener
            auth.subscribeToUserData(username, (newData) => {
                console.log("Data received from Cloud.");
                applyTheme();
                renderProjects(); // This will refresh the list when cloud data arrives
                if(typeof translatePage === 'function') translatePage();
            });

            // Second: Trigger a cloud download specifically for login
