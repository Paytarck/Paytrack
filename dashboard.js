...
document.getElementById('editProjectForm').addEventListener('submit', async (e) => {
    ...
    const btnText = document.getElementById('saveEditBtnText');
    ...
    try {
        ...
        const now = Date.now(); // The "Sync Key" for other devices

        // 1. Update the Global Project List (for the Name)
        ...
        // 2. Update the Deep Project Data (Date, Cost, Receipts)
        ...
        if (projectType === 'installment') {
            ...
            // This is the metadata shown in the "Agreement" modal
            projectData.projectMetaData = {
                ...
                receipts: [...editTempImages] // Synced images
            };
            
            projectData.projectName = newName;
            projectData.totalAmount = newTotal;
            
            // Recalculate totals for the progress bar
        ...
        // Attach the timestamp so other devices trigger an update
        projectData.lastUpdated = now;
        ...
        // Sync Name change to User Profile
        await auth.syncDataToCloud(); 

        // Sync Metadata, Date, and Receipts to Project Card
        const fullPackage = {
        ...
        await auth.updateGlobalCard(currentProject, fullPackage);

        // Success UI
        document.getElementById('editProjectModal').classList.add('hidden');
        renderProjects();
        alert("Sync Complete: All devices will now see these changes.");
