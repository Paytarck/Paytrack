...
const checkBiometric = async () => {
    ...
    // In Capacitor, we don't check for 'window.PublicKeyCredential'
    // We check if the native plugin is available and enabled in settings
    ...
    if (isEnabled && result.isAvailable) {
        ...
        // Auto-trigger the fingerprint popup when the lock screen opens
        try { 
            await triggerBiometricAuth(); 
        } catch (e) { 
            console.log("Auto-biometric blocked or cancelled"); 
        }
    }
};

// --- IMPROVED BIOMETRIC LOGIC FOR ALL SENSOR TYPES ---
const triggerBiometricAuth = async () => {
    const { NativeBiometric } = Capacitor.Plugins;
    
    try {
        // 1. Check availability and get the type (Fingerprint, Face, etc.)
        const result = await NativeBiometric.isAvailable();
        
        if (!result.isAvailable) return;

        // 2. Trigger the system prompt
        // Using a slight delay ensures the Native OS overlay doesn't glitch on under-screen sensors
        setTimeout(async () => {
            try {
                await NativeBiometric.verifyIdentity({
                    ...
                    maxAttempts: 3 // Standard for most devices
                });

                // SUCCESS
                colorDots('success');
                showFeedback("Verified!", "success");
                ...
                showFeedback("Try PIN", "error");
                shakeUI();
            }
        }, 300); // 300ms delay helps in-display sensors initialize
